...
async function loadPhotoMap() {
    try {
        ...
        // Usar la API para listar imágenes
        ...
        console.log(`API: Cargadas ${data.total} imágenes desde el servidor`);

        // Mapear filenames a índices de foto (foto0001.webp -> 1)
        photos = data.images.map(img => img.filename);
        photosLoaded = true;
        return true;
    } catch (error) {
        console.error('Error loading photos from API:', error);
        // Fallback: usar nombres estándar
        photos = Array.from({length: 300}, (_, i) => `foto${String(i + 1).padStart(4, '0')}.webp`);
        photosLoaded = true;
        return false;
    }
}
...
const STORAGE_KEY = 'boda_francisco_rossy_photo_selections';
let photoSelections = {};
...
function renderGallery() {
    ...
    photos.forEach((photo, index) => {
        ...
        if (canvas) {
            // Usar auth para obtener URL con token
            const imageUrl = auth.getImageUrl(photo) || `/api/images/${photo}`;
            loadImageToCanvas(imageUrl, canvas, index + 1).catch(err => {
...
function openModal(index) {
    ...
    const imageUrl = auth.getImageUrl(photos[index]) || `/api/images/${photos[index]}`;
    loadImageToCanvas(imageUrl, modalCanvas, index + 1).catch(err => {
...
function navigatePhoto(direction) {
    ...
    const proceed = () => {
        ...
        if (newIndex !== null) {
            ...
            const imageUrl = auth.getImageUrl(photos[newIndex]) || `/api/images/${photos[newIndex]}`;
            loadImageToCanvas(imageUrl, modalCanvas, newIndex + 1).catch(err => {
...
function exportToJSON() {
    ...
    a.href = url;
    a.download = `seleccion-francisco-rossy-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
...
function generateTextSummary() {
    const stats = getStats();
    let summary = '💒 SELECCIÓN DE FOTOS - BODA FRANCISCO Y ROSSY\n';
    summary += '═══════════════════════════════════════\n\n';
...
// FEEDBACK MANAGEMENT
// ========================================
const FEEDBACK_KEY = 'boda_francisco_rossy_feedback';
let feedbackData = {
    video: [],
    photos: []
};
